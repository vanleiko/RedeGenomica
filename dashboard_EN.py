...
# período das frequências do último mês do card 3 
...
df_genomasLaboratorio = df_genomasLaboratorio.rename(columns={'Data': 'Date','Laboratório': 'Laboratory','Período': 'Period','Quantidade': 'Quantity','Acumulado': 'Cumullative'})
...
df_variantes = df_variantes.replace({'Norte': 'North', 'Nordeste': 'Northeast', 'Sudeste': 'Southeast', 'Centro-oeste': 'Central-west', 'Sul': 'South', 'Outras': 'Other', 'Outras': 'Others'})
df_variantes = df_variantes.replace(regex={'Fev': 'Feb', 'Abr': 'Apr', 'Mai': 'May', 'Ago': 'Aug', 'Set': 'Sep', 'Out': 'Oct', 'Dez': 'Dec', 'Gama': 'Gamma', 'Alfa': 'Alpha'})
df_variantes = df_variantes.rename(columns={'Data': 'Date', 'Período': 'Period', 'Região': 'Region', 'Estado': 'State', 'Laboratório': 'Laboratory', 'Variante': 'Variant', 'Variantes relevantes': 'Relevant variants', 'Quantidade': 'Quantity'})

df_linhagens = pd.read_csv('datasets/dfLinhagens.csv')
df_linhagens = df_linhagens.replace({'Norte': 'North', 'Nordeste': 'Northeast', 'Sudeste': 'Southeast', 'Centro-oeste': 'Central-west', 'Sul': 'South', 'Outras': 'Others'}) 
df_linhagens = df_linhagens.replace(regex={'Fev': 'Feb', 'Abr': 'Apr', 'Mai': 'May', 'Ago': 'Aug', 'Set': 'Sep', 'Out': 'Oct', 'Dez': 'Dec', 'Gama': 'Gamma', 'Alfa': 'Alpha'})
df_linhagens = df_linhagens.rename(columns={'Data': 'Date', 'Período': 'Period', 'Região': 'Region', 'Estado': 'State', 'Laboratório': 'Laboratory', 'Linhagens relevantes': 'Relevant lineages', 'Quantidade': 'Quantity'})


df_mapa = pd.read_csv('datasets/dfMapa.csv')
df_mapa = df_mapa.rename(columns={'Estado': 'State', 'Genomas sequenciados': 'Genomes sequenced', 'Genomas/100 mil casos': 'Genomes/100K cases'})
...
df_omicron_mapa = df_omicron_mapa.rename(columns={'Estado': 'State', 'Amostras': 'Samples', 'Última coleta': 'Last sampling on',
						  ...
						  '% Provável BA.4 ou BA.5': '% Likely BA.4 or BA.5',
					          '% Provável XBB': '% Likely XBB'})
...
# card 3
# index_omicron = df_linMes[df_linMes['Linhagem'] == 'BA.1+B.1.1.529 (Omicron)'].index.item()
...
layout_barplot = {'xaxis_title':'<b>Sampling period</b>', 
                  ...
                  'xaxis_title_font_size':16, 'yaxis_title_font_size':16,
                  'title_x':0.5, 'title_font_size':22, 'title_font_family':'sans-serif', 
                  'legend_font_size':13, 
                  'plot_bgcolor':'white', 'paper_bgcolor':'white'}
layout_area = {'xaxis_title':"<b>Sampling period</b>",
               ...
               'xaxis_title_font_size':16, 'yaxis_title_font_size':16,
               'title_x':0.5, 'title_font_size':22, 'title_font_family':'sans-serif',
               'legend_font_size':13, 'legend_title_font_size':12}
legend={'orientation':'h', 'xanchor':'center', 'yanchor':'bottom', 'x':0.5, 'y':1.0}
############# FIM ABA VOCs/VOIs #############
...
legenda_aba_omicron={'orientation':'h',
                     ...
                     'yanchor':'bottom', 
                     'x':0.5, 'y':1.05}
layout_barplot_line_aba_omicron = {'xaxis_title':'<b>Sampling date</b>', 'xaxis_title_font_size':16, 
                                   'xaxis_tickangle':-45, 'xaxis_tickformat':'%d-%m-%Y',
                                   'yaxis_title_font_size':16,
                                   'legend_font_size':14, 'font_family':'sans-serif',
...
## Callback 1 - VOCs/VOIs
@app.callback([Output(component_id='row1_bar', component_property='figure'),
               Output(component_id='row1_area', component_property='figure'),
               Output(component_id='row2_bar', component_property='figure'),
               Output(component_id='row2_area', component_property='figure')],
               [Input(component_id='regiao_dropdown1', component_property='value'),
                Input(component_id='estado_dropdown1', component_property='value')])
def update_variantes(regiao1, estado1):
  ### REGIÃO ###
  if regiao1 == 'Brazil':
    df_voc_barplot = df_brasil_variantes_barplot
    df_voc_area = df_brasil_variantes_area
  else:
  ...
  row1_area.update_xaxes(tickangle=320, showgrid=True, gridcolor='lightgray', tickfont_size=14, showline=True, linewidth=0.5, linecolor='gray')
  ...
  row2_bar = px.bar(df_estado_voc_barplot,
                    ...
                    hover_data={'Relevant variants': False,
                                ...
                                'Laboratory': True,
                                'Quantity': True}, 
                    color_discrete_map=colors_variantes, category_orders=orders_variantes, height=600,
  ...
  return row1_bar, row1_area, row2_bar, row2_area


## Callback 2 - Linhagens
@app.callback([Output(component_id='row5_bar', component_property='figure'),
               Output(component_id='row5_area', component_property='figure'),
               Output(component_id='row6_bar', component_property='figure'),
               Output(component_id='row6_area', component_property='figure')],
               [Input(component_id='regiao_dropdown2', component_property='value'),
                Input(component_id='estado_dropdown2', component_property='value')])

def update_linhagens(regiao2, estado2): 
  ### REGIÃO ###
  if regiao2 == 'Brazil':
  ...
  row5_bar = px.bar(df_lin_barplot,
                    ...
                    opacity=0.95, title=f'{regiao2}<br><sup>Relevant lineages</sup>', height=610)
  ...
  row6_bar = px.bar(df_estado_lin_barplot,
                    ...
                    hover_data={'Relevant lineages': False,
                                ...
                                'Quantity': True},
                    color_discrete_map=colors_linhagens, category_orders=orders_linhagens,
                    opacity=0.95, title=f'{estado2}<br><sup>Relevant lineages</sup>', height=580)
  ...
  return row5_bar, row5_area, row6_bar, row6_area


## Callback 3 - Aba Ômicron
